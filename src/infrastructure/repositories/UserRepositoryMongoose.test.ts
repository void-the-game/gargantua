...
describe('UserRepositoryMongoose', () => {
  ...
  it('should throw an error if token is not provided', async () => {
    await expect(repo.verifyEmail('userId', '')).rejects.toThrow(
      'Token is required'
    )
  })
  ...
  it('should throw if token is not found', async () => {
    ...
    await expect(repo.verifyEmail('userId', 'sometoken')).rejects.toThrow(
      'Token not found'
    )
  })

  it('should throw if token does not match', async () => {
    ;(TokenModel.findOne as jest.Mock).mockResolvedValue({
      token: 'othertoken',
    })
    await expect(repo.verifyEmail('userId', 'sometoken')).rejects.toThrow(
      'Invalid token'
